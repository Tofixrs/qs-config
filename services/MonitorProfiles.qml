...
Singleton {
	...
	readonly property string hyprMonitorConfigPath: `${Paths.strip(Paths.home)}/.config/hypr/hyprland_monitor.conf`
	...
	function buildMonitorLine(draft: var): string {
		if (draft.enabled === false)
			return `monitor=${draft.name},disable`;
		...
		return `monitor=${draft.name},${resolution}${refreshPart},${position},${scale}${extras.length > 0 ? `,${extras.join(",")}` : ""}`;
	}
